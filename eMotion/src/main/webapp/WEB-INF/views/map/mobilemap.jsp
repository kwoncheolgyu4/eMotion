      ...
      <div class="board-container">
        ...
        /* 내 Key : n9pFS1xDL83iAckIvHawc1M0unL7Z3LR2LqJ1kjN */
                ...
                navigator.geolocation.getCurrentPosition(
                    async function (position) { 
                        ...
                        try {
                            ...
                            alert("역지오코딩 실패:", error);
                        }
                    },
                    function (error) {
                        alert("위치를 가져오지 못했습니다.", error);
                    }
                );
            } else {
		    	...
		    	$.ajax({
		    		...
		    		success: function(statid)
		    		{
		    		    if (statid === null || statid === "") {
		    		        ...
		    		        return;  // statid가 null일 경우 차트 생성 요청을 중단합니다.
		    		    }
            ...
            async function updateStationPoints() {
                ...
                for (const station of nearestPoints) {
                    try {
                        ...
                        stationmarker.addListener("touchend", async function (evt) {
            ...
            async function updateStationPointsFromLine(routePolyline) {
                ...
                centercopyStationPoints.forEach(function(marker)
                {
    	           const matchingMarkers = StationMarkers
    	                // 마커가 선택된 타입일 경우 보여주기, 아니면 숨기기
    	           matchingMarkers.forEach(function(stationMarker) {
    	                stationMarker.setMap(null); // 선택되지 않은 타입일 경우 마커 숨기기
    	             });
            ...
            // 초기값 설정
                ...
                // 1. 지도 띄우기
                    ...
                    var isDragging = false; // 드래그 여부
                    var startX = 0, startY = 0; // 터치 시작 좌표
                    var touchstartTime = 0; // 터치 시작 시간
                    const dragThreshold = 5; // 드래그로 간주할 최소 거리 (픽셀)
                    const timeThreshold = 100; // 클릭으로 간주할 최대 시간 (밀리초)
                    
                    
                    map.addListener("touchstart", function (e) {
                        // // 터치 시작 시 초기화
                        startX = e.x ; // 화면 좌표 (뷰포트 기준)
                        startY = e.y ; // 화면 좌표 (뷰포트 기준)
                        touchstartTime = Date.now(); // 터치 시작 시간 기록
                        isDragging = false; // 초기화
                    });
                    
                    map.addListener("touchmove", async function (evt) {
                        const deltaX = Math.abs(e.x - startX);
                        const deltaY = Math.abs(e.y - startY);
                        // 일정 거리 이상 이동하면 드래그로 간주
                        if (deltaX > dragThreshold || deltaY > dragThreshold) {
                            isDragging = true;
                        }
                    });
                    map.addListener("touchend", async function (evt) {
                        const endTime = Date.now();
                        const elapsedTime = endTime - touchstartTime; // 터치 시간 계산
				                        ...
				                        '<div class="table-responsive">' +
				                            '<table class="table">' +
				                                ...
				                                '<tr>' +
					                                '<th>TMAP CODE</th>' +
					                                '<td>' +
					                                    '<a href="' + mobileqrLink + '" target="_blank">여기를 클릭하세요</a>' + 
					                                '</td>'+
				                                '</tr>' +
				                                '<tr>' +
				                                    '<th>NAVER MAP</th>' +
                ...
                window.addMarker = function(status, lat, lon, tag) {
                        ...
                        if (status === "llStart") {
                            ...
                            StartMarker.addListener("touchend", function(evt) {
                        ...
                        EndMarker.addListener("touchend", function(evt) {
                        ...
                        if (StartPoint.lat !== null && StartPoint.lon !== null && EndPoint.lat !== null && EndPoint.lon !== null) {
                            ...
                            mobileqrLink = "tmap://route?startx=" + StartPoint.lon + "&starty=" + StartPoint.lat +
                ...
                if (index !== -1) {
    	            ...
    	            ViaPoint.forEach(function(point, index) {
    	                if (point.name && point.lat && point.lon) {
    	                    mobileqrLink += "&via" + (index + 1) + "name=" + encodeURIComponent(point.name) + 
            ...
            function swapValues() {
                ...
                if (StartPoint.lat && StartPoint.lon && EndPoint.lat && EndPoint.lon) {
                    ...
                    mobileqrLink = "tmap://route?startx=" + 
                        StartPoint.lon + "&starty=" + StartPoint.lat + 
                        "&goalx=" + EndPoint.lon + "&goaly=" + EndPoint.lat;
                    ...
                    // 경유지 추가
       		        if (ViaPoint && ViaPoint.length) {
       		            ...
       		            naverMapLink = "https://map.naver.com/index.nhn"
       		            naverMapLink += startCoord.x + "," + startCoord.y + "," + encodeURIComponent(StartPoint.name) + ",,SIMPLE_POI/";
       		            naverMapLink += endCoord.x + "," + endCoord.y + "," + encodeURIComponent(EndPoint.name) + ",,SIMPLE_POI/";
